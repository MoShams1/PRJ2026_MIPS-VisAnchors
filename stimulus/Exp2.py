...
date = sup.get_date()
time = sup.get_time()
file_name = f"exp02v2_{subject}_{date}_{time}.json"
...
primary_half = 3                # primary frame (square): 6 dva total width
aux_radii = [1.5, 3.0, 4.5]     # auxiliary frame (circle) radii: small/medium/large
                                # small = inside primary, medium = inscribed, large = around primary

motion_path_length = primary_half * 2  # dva
...
# (primary_visible, aux_visible, aux_size_index)
condition_specs = {
    1: (True,  True,  0),   # primary + aux small
    2: (True,  True,  1),   # primary + aux medium
    3: (True,  True,  2),   # primary + aux large
    4: (False, True,  0),   # aux small only
    5: (False, True,  1),   # aux medium only
    6: (False, True,  2),   # aux large only
    7: (True,  False, 0),   # primary only
}
cnd_array_base = np.array([1, 2, 3, 4, 5, 6, 7])
nrep_per_trial = 7
...
probe = visual.Circle(win,
                      radius=probe_radius,
                      pos=[probe_x, probe_y])
primary_frame = visual.Rect(win,
                            width=2*primary_half, height=2*primary_half,
                            lineColor=frame_color, fillColor=None,
                            lineWidth=frame_linewidth)
aux_frame = visual.Circle(win,
                          radius=aux_radii[0],
                          lineColor=frame_color, fillColor=None,
                          lineWidth=frame_linewidth)
...
for itrial in range(ntrials):
    ...
    primary_visible, aux_visible, aux_size_idx = condition_specs[cnd]
    current_aux_radius = aux_radii[aux_size_idx]
    aux_frame.radius = current_aux_radius
        ...
        for ileg in range(2):
            ...
            for xind, xval in enumerate(motion_vector[0:-1]):
                for irep in range(flash_dur_frames):
                    ...
                    if primary_visible:
                        primary_frame.pos = (0 + xval, 0)
                        primary_frame.draw()
                    if aux_visible:
                        aux_frame.pos = (0 + xval, 0)
                        aux_frame.draw()
